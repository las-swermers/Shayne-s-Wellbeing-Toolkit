...
| Rounds | account_id + round_id | baseline start/end, intervention start/end, mode, answers, selected strategy IDs, status, version |
| Entries | account_id + round_id + waking_date | entry_id, raw log values, derived values, calculation version, strategy IDs completed, created_at, updated_at, version, deletion state |
