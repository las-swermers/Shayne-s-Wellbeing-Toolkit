...
const band=d.getElementById('wellbeingBand'),pause=d.getElementById('pauseWellbeing');
...
pause.click();assert.equal(pause.getAttribute('aria-pressed'),'true');assert.ok(band.classList.contains('is-paused'));
pause.click();assert.equal(pause.getAttribute('aria-pressed'),'false');
frames.clear();
...
dom.window.close();
console.log('Landing checks passed: dropdown, ribbon pause, accessible duplicates, jump clearance, Escape/focus, skip once, Back, reduced motion and missing-art fallback.');
