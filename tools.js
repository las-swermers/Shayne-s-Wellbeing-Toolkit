...
/* Sleep Lab opens the original sleeplab.html uploaded at the repository root. */
const tools = [
  { id: 'sleep-lab', name: 'Sleep Lab', cat: 'Experiment', blurb: 'A practical experiment for noticing what helps you rest.', href: 'sleeplab.html', access: 'open', icon: 'moon' },
...
shelf.innerHTML = tools.map(tool => {
  ...
  const content = `<span class="icon">${icons[tool.icon]}</span><h2>${tool.name}</h2><p>${tool.blurb}</p><span class="button">Read more</span>`;
